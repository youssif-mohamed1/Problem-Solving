...
bool check(string x, string y, int i, int j, ll l) {
	if (i == 0) {
		return (x[j + 1] == '1' && y[j + 1] == '1');
	}
	return (x[i - 1] == '0' && y[i - 1] == '0');
}
...
void solve() {
	...
	if (ans % 2 == 0 && ans!=2) {
		cout << ans / 2 << " " << ans / 2; return;
	}
	for (ll i = 2; i * i <= ans; i++) {
		if (ans % i == 0) {
			if (gcd((ans / i), (ans - (ans / i))) != 1) {
				cout << ans / i << " " << ans - (ans / i); return;
			}
			if (gcd((ans / (ans / i)), (ans - (ans / (ans / i)))) != 1) {
				cout << ans / (ans / i) << " " << ans - (ans / (ans / i)); return;
			}
		}
	}
	cout << -1;
