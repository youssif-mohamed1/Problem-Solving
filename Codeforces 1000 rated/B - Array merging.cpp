...
void solve() {
    ...
    fpi(1, n) {
        if (a[i] == a[i - 1]) {
            ...
            m[a[i - 1]].first = max(m[a[i - 1]].first, cnt);
            cnt = 1;
        }
    }
    m[a[n - 1]].first = max(m[a[n - 1]].first, cnt);
    cnt = 1;
    fpi(1, n) {
        if (b[i] == b[i - 1]) {
            cnt++;
            ...
            m[b[i - 1]].second = max(m[b[i - 1]].second, cnt);
            cnt = 1;
        }
    }
    m[b[n - 1]].second = max(m[b[n - 1]].second, cnt);
    ll maxi = 0;
    for (auto it : m) {
