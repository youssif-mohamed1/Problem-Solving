...
//ll generateMaxDistinctGCDPermutation(ll n) { map<int, bool>m; for (int i = 1; i <= n; i++) { if (m[i] == 0) { cout << i << " "; m[i] = 1; int j = i; while (j * 2 <= n) { j *= 2; cout << j << " "; m[j] = 1; } } } }
using namespace std;
//=================================================================================MY Segment Tree======================================================================================================================================================================
...
//=======================================================================================================================================================================================================================================================================
ll eatenCookies(ll nxt, ll prev, ll d) {
	return (nxt - prev - 1) / d;
}
void solve() {
	ll n, m, d,totall=0,mini=1e9,count=0;
	cin >> n >> m >> d;
	deque<ll>v(m);
	fpi(m) {
		cin>>v[i];	
	...
	v.push_back(n + 1);
	for (int i = 1; i <= m + 1; i++) {
		totall += eatenCookies(v[i],v[i-1],d) + 1;
	}
	totall -= 2; // as we add 2 virtual points
	for (int i = 1; i <= m; i++) {
		ll ans = totall;
		ans -= eatenCookies(v[i], v[i - 1], d); // removing what is eaten before this point
		ans -= eatenCookies(v[i + 1], v[i], d); // removing what is eaten after this point
		ans += eatenCookies(v[i + 1], v[i - 1], d);// adding what is eaten without this point
		if (mini > ans) {
			count = 1;
			mini = ans;
		}
