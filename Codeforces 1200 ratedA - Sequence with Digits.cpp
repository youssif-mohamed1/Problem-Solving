...
using namespace std;
//=====================================================================================================================================================================================================================================
pair<int, int> min_max_element(string x) {
	int mini = 10, maxi = 0, l = x.size();
	fpi(l) {
		mini = min(mini, int(x[i]) - '0');
		maxi = max(maxi, int(x[i]) - '0');
	}
	return { mini,maxi };
}
void solve() {
	...
	--k;
	while (k--) {
		pair<int, int>it = min_max_element(to_string(n));
		if (it.first == 0)
			break;
		n += static_cast<long long>(it.first) * static_cast<long long>(it.second);
	}
	cout << n;
}// O((k-1) *|n|) 1e18 10 0*x=0
