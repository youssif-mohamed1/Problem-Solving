...
bool valid(vll s,vll sum,ll n,ll x,ll mid) {
	for (int i = 1; i <= n - mid+1; i++) {
		if (sum[i + mid-1] - sum[i] == mid - 1) {
...
void solve() {
	...
	fpi(1, n+1)sum[i] += sum[i - 1];
	ll l = -1, r = 1e9+1, mid, ans=0;
	while (l < r - 1) {
		mid = l + r >> 1;
		if (valid(s, sum, n, x, mid)) {
			l = mid;
			ans = mid;
			continue;
		}
