...
bool Greater(char a, char b) { return a > b; }
...
//=======================================================================================================================================================================================================================================================================
void generateFacto(vector<ll>& v) {
	for (int i = 2; i <= 9; i++) {
...
void solve() {
	...
	vector<ll>digits;
	generateFacto(factorials);
	fpi(n) {
		if (x[i] != '0' && x[i] != '1') {
			digits.push_back(int(x[i]) - '0');
		}
	}
	string z;
	ll l = digits.size();
	vector<ll>distinct = { 7,5,3,2 };
	fpi(l) {
		int index = digits[i] - 2;
		ll f = factorials[index];
		string y;
		for (int j = 0; j <= 3;) {
			if (f % factorials[distinct[j]-2] == 0) {
				f /= factorials[distinct[j] - 2];
				y += to_string(distinct[j]);
			}
			else {
				j++;
			}
