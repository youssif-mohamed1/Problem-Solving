...
using namespace std;
//=====================================================================================================================================================================================================================================
bool isDiagonal(int i, int j, int x, int y) {
	return  (i - x) == (j - y) || (i - x) == (-1)*(j - y);
}
void solve() {
	int x1, x2, y1, y2;
	cin >> x1 >> y1 >> x2 >>y2;
	if (isDiagonal(x1, y1, x2, y2)) {
		cout << x1 << " " << y2 << " " << x2 << " " << y1; return;
	}
	if (x1 == x2) {
		cout << x1 + (max(y2,y1)-min(y2,y1)) << " " << y1 << " " << x2 + (max(y2, y1) - min(y2, y1)) << " " << y2; return;
	}
	if (y1 == y2) {
		cout << x1 << " " << y1 + (max(x2, x1) - min(x2, x1)) << " " << x2 << " " << y2 + (max(x2, x1) - min(x2, x1)); return;
	}
	cout << -1;
}
