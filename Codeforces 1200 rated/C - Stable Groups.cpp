...
void solve() {
    ...
    fpi(0,n)cin>>v[i];
    sort(all(v));
    groupes.push_back(v[0]);
    fpi(1,n){
        if(v[i]-v[i-1]<=x){
            groupes.push_back(v[i-1]);
            groupes.push_back(v[i]);
        }else {
                m.push_back({groupes[0], groupes[groupes.size() - 1]});
                groupes.clear();
                groupes.push_back(v[i]);
        }
    }
    ll u=groupes.size();
    if(u>=1){
        m.push_back({groupes[0],groupes[u-1]});
    }
    ll l=m.size(),ans=l;
    vll b;
    ll c=0;
    fpi(1,l) {
        b.push_back(ceil((m[i].first - m[i - 1].second) / static_cast<long double>(x))-1);
        c++;
   }
    sort(all(b));
    fpi(0,c){
