...
void solve() {
	...
	ll s = sz(minis);
	ll ans = llabs(x.first - y.first) + llabs(x.second - y.second), miniA = 1e18, miniB = 1e18;
	bool flag = 0;
	for (int i = 0; i < s; i++) {
		miniA = min(miniA, llabs(x.first - minis[i].first) + llabs(x.second - minis[i].second));
		miniB = min(miniB, llabs(minis[i].first - y.first) + llabs(minis[i].second - y.second));
		flag = 1;
	}
	if (++a <= k && ++b <= k) {
		cout << 0; return;
	...
	if (flag) {
		cout << min(ans, miniA + miniB); return;
	}
	cout << ans;
}
