...
void generateNumbers(int still, int current, vector<int>& v, map<int, int>& m, double& n) // for generating truth tables
{
	if (still == 0)
	{
		v.push_back(current);
...
bool valid(long long n, long long x)
{
	long long temp = cbrt(x - n);
	return (pow(temp, 3) == (x - n));
}
...
void test()
{
	...
	cin >> x;
	bool flag = false;
	long long check,diff,temp,cbr = cbrt(x);
	long long  i = 1;
	if (x == 1) { goto here; }
	else if (x == 2) { goto there; }
	for (; i*i*i < x; i++)
	{
		 check = pow(i, 3);
		 diff = x - check;
