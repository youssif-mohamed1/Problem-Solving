...
void sieveOfEratosthenes(vector<ll>& composite, int sz) { composite[0] = composite[1] = 1; for (int i = 2; i * i < sz; i++) { if (!composite[i]) { for (int j = i * i; j < sz; j += i) { composite[j] = 1; } } } }// nlog(log(n))
ll countOnes(ll n) {ll count = 0;while (n > 0) {if (n & 1) {count++;}n >>= 1;}return count;}
using namespace std;
//=====================================================================================================================================================================================================================================
...
int find_min(int based, int l, int r, int k) {
	if (l == r) {
		return l;
	}
	int m = l + r >> 1;
	...
	int right = based << 1 | 1;
	if (k <= seg[left]) {
		return find_min(left, l, m, k);
	}
	else {
		return find_min(right, m + 1, r, k);
	}
}
int find_max(int based, int l, int r, int k) {
	if (l == r) {
		return l;
	}
	int m = l + r >> 1;
	...
	int right = based << 1 | 1;
	if (k <= seg[right]) {
		return find_max(right, m + 1, r, k);
	}
	else {
		return find_max(left, l, m, k);
	}
}
...
void solve() {
	...
	build(1, 0, n - 1);
	fpi(n) {
		ll first = find_min(1, 0, n - 1, arr[i]);
		ll last = find_max(1, 0, n - 1, arr[i]); 
		ans[arr[i]-1] = 2 * (last - first + 1);
	}
	for (auto it : ans) {
		cout << it << " ";
	}
